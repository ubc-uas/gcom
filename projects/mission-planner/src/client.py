...
def create_waypoint(
    id=int(MAVLink.MAV_CMD.WAYPOINT), lat=0, lng=0, alt=0, p1=0, p2=0, p3=0, p4=0
):
    wp = Locationwp()
    ...
    if cs.mode == "MANUAL":  # Safety Manual Mode Switch
        ...
        if cmd == "NEW_MISSION":
            ...
            for i in range(0, numwp):
                try:
                    old_mission.append(
                        MAV.getWP(MAV.sysidcurrent, MAV.compidcurrent, i)
                    )
                except:
            ...
            if len(argv) != 1:
                ...
                takeoff = create_waypoint(
                    int(MAVLink.MAV_CMD.TAKEOFF), cs.lat, cs.lng, takeoffalt
                )
                loiter_unlim = create_waypoint(
                    int(MAVLink.MAV_CMD.LOITER_UNLIM), cs.lat, cs.lng, 0, p3=1
                )
            ...
            MAV.doCommand(
                MAVLink.MAV_CMD.COMPONENT_ARM_DISARM, int(argv[0]), 0, 0, 0, 0, 0, 0, 0
            )
            ...
            rtl_altitude = (
                float(argv[0]) * 100
            )  # argv[0] (meters) -> RTL_ALT param (centimeters)
            if MODE == "plane":
